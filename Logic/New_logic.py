...
class The_Lich_King(Enemy):
    ...
    def battle(self, attack, hero):
        ...
        if attack == 1:
            if self.get_distance() == 1:
                ...
                result_attack = ('Ты наносишь удар, и меч пронзаёт Лича, но его тело не кровоточит. Тёмный свет'
                                 ' окружает его, и он лишь улыбается, словно не ощущая боли.\n')
            elif self.get_distance() == 2:
                self.set_HP(self.get_HP() - (hero.get_AP() * self.coof_AP_hero) / 2)
                hero.set_HP(hero.get_HP() - ((self.get_AP() - 2) / 2))
                result_attack = ('Твой меч сталкивается с Личом, но он не оставляет ран. Его присутствие — как стена,'
                                 ' защищающая его от боли. Ты чувствуешь холод, и это ощущение только усиливает твою'
                                 ' слабость.\n')
            else:
                hero.set_HP(hero.get_HP() - (self.get_AP() - 2))
                result_attack = ('Ты замахиваешься, но меч не достигает Лича, его туманная аура просто отталкивает твоё'
                                 ' оружие. Ты чувствуешь, как его магия проникает в твоё тело, и мороз сковывает руки.\n')
            if self.get_HP() <= 0:
                self.set_Dead()
                hero.set_HP(hero.max_HP)
                return ('Ты вонзаешь меч в Лича, и его тёмная аура начинает исчезать. Он падает на колени, и с'
                        ' последним взглядом полного отчаяния, он растворяется в воздухе, его ледяная сила исчезает'
                        ' навсегда.')
            if hero.get_HP() <= 0:
                hero.set_dead()
            ...
            if self.get_distance() == 2:
                self.set_HP(self.get_HP() - (hero.get_AP() * self.coof_AP_hero))
                result_attack = ('Ты выстреливаешь болт, и он вонзается в Лича. В его глазах появляется тень, но он'
                                 ' отходит в сторону, вновь окружая себя темной энергией.\n')
            elif self.get_distance() == 3:
                self.set_HP(self.get_HP() - (hero.get_AP() * self.coof_AP_hero) / 2)
                hero.set_HP(hero.get_HP() - ((self.get_AP() - 2) / 2))
                result_attack = (
                    'Ты выстреливаешь, и болт пробивает ауру Лича, но он продолжает стоять, а ответная атака'
                    ' холодной магии поражает тебя, замораживая твою руку.\n')
            else:
                hero.set_HP(hero.get_HP() - (self.get_AP() - 2))
                result_attack = ('Ты целишься в Лича, но его тёмная магия отводит твой арбалетный болт в сторону.'
                                 ' Ответная атака ледяной энергии поражает тебя, сковывая твоё тело.\n')
            if self.get_HP() <= 0:
                self.set_Dead()
                hero.set_HP(hero.max_HP)
                return ('Ты выстреливаешь болт в Лича, и его ледяная аура трескается. Он падает на землю,'
                        ' а его магия исчезает. Лич исчезает, и ты ощущаешь, как холодная тень покидает этот мир.')
            if hero.get_HP() <= 0:
                hero.set_dead()
            ...
            if self.get_distance() == 3:
                self.set_HP(self.get_HP() - (hero.get_AP() * self.coof_AP_hero))
                result_attack = ('Ты направляешь огненный шар в Лича, и он пронзается через его ледяной барьер,'
                                 ' нанося урон. Однако, его сила всё ещё кажется непобедимой.\n')
            elif self.get_distance() == 1:
                self.set_HP(self.get_HP() - (hero.get_AP() * self.coof_AP_hero) / 2)
                hero.set_HP(hero.get_HP() - ((self.get_AP() - 2) / 2))
                result_attack = ('Ты запускаешь огненный шар, и он обжигает Лича, но он лишь рассекает воздух вокруг'
                                 ' своей туманной фигуры. Холодная аура наполняет твои лёгкие, и ты чувствуешь боль.\n')
            else:
                hero.set_HP(hero.get_HP() - (self.get_AP() - 2))
                result_attack = ('Ты выпускаешь огненный шар, но Лич поглощает пламя своим ледяным щитом.'
                                 ' Ожог не прекращается, и ты ощущаешь холод, который сковывает твоё тело..\n')
            if self.get_HP() <= 0:
                self.set_Dead()
                hero.set_HP(hero.max_HP)
                return ("Ты отправляешь огненный шар в Лича, и его тёмная магия не выдерживает удара. Он падает,"
                        " и его ледяная аура исчезает, оставляя пустоту, в которой некогда жил Лич.")
            if hero.get_HP() <= 0:
                hero.set_dead()
            ...
            if self.get_HP() <= 0:
                self.set_Dead()
                return ("Мощное заклинание проходит Лича насквозь, пробивая его барьер и броню. Он падает на колени, его"
                        "глаза меркнут, а леденая аура начинает таить.\n")
            return (f"{result_attack}"
                    f"HP у вас: {hero.get_HP()}\n"
                    f"HP у Лича: {self.get_HP()}\n"
                    f"{self.set_distance_battle(random.randint(1, 3))}\n")
    ...
    def set_distance_battle(self, new_distance):
        if self.get_distance() == new_distance:
            ...
            self.set_distance(new_distance)
            if new_distance == 2:
                return (f"Он не движется быстро, но словно растворяется в воздухе, его фигура изменяется,"
                        f" и он появляется на более безопасном расстоянии. Ледяные осколки взлетают вокруг."
                        f" Дистанция до него: {new_distance}")
            else:
                return (f"Он резко отступает, делая шаг назад и оставляя тебя на более дальнем расстоянии."
                        f" Он подготавливает свои магические силы для следующей атаки. Дистанция до него:"
            ...
            self.set_distance(new_distance)
            if new_distance == 3:
                return (f"Он отступает, его взгляд всё равно направлен на тебя. Он готовит магическую атаку,"
                        f" чтобы продолжить разрушать твою душу.: {new_distance}")
            else:
                return (f"Он не торопится, но делает шаг назад, не давая тебе времени на манёвры."
                        f" Он готовится провести свою атаку."
                        f" Дистанция до него: {new_distance}")
            ...
            self.set_distance(new_distance)
            if new_distance == 2:
                return (f"Он неспешно сокращает расстояние между вами, его тёмная магия охватывает воздух,"
                        f" и ты ощущаешь её ледяную хватку.: {new_distance}")
            else:
                return (f"Он невидимо сокращает дистанцию, поглощая свет вокруг. "
                        f"Его сила увеличивается, а твоя боль усиливается от холода."
                        f" Дистанция до него: {new_distance}")
